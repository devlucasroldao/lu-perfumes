                ...
                <div key={p.id} style={{ ...styles.item, opacity: p.ativo ? 1 : 0.5 }}>
                  ...
                  <div style={styles.itemAcoes}>
                    <button style={styles.btnToggle} onClick={() => toggle(p)}>
                      {p.ativo ? '⏸ Desativar' : '▶️ Ativar'}
                    </button>
                    <button style={styles.btnEditar} onClick={() => editar(p)}>✏️ Editar</button>
                    <button style={styles.btnExcluir} onClick={() => excluir(p.id)}>🗑</button>
                  </div>
                </div>
              ))}
            </div>
...
const styles = {
  page: { background: 'var(--bege, #F3EFE9)', minHeight: '100vh' },
  header: { background: '#fff', padding: '20px 32px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' },
  ...
  input: { padding: '10px 14px', borderRadius: 8, border: '2px solid #eee', fontSize: 14, outline: 'none', fontFamily: 'inherit' },
  ...
  itemInfo: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
